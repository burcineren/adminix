...
export function Sidebar({ resources, title = "AutoAdmin", logo, plugins, showDashboard = true }: SidebarProps) {
    const { activeResource, setActiveResource, darkMode, toggleDarkMode, sidebarOpen, toggleSidebar } =
        useAdminStore();
        ...
        <>
            ...
            >
                ...
                <div className="flex h-14 items-center justify-between gap-2 border-b border-[hsl(var(--border))] px-4">
                    ...
                    <Button variant="ghost" size="icon" onClick={toggleSidebar} className="lg:hidden h-7 w-7">
                        <X className="h-4 w-4" />
                    </Button>
                </div>
                ...
                <nav className="flex-1 overflow-y-auto py-3">
                            ...
                            return (
                                <li key={res.name}>
                                    ...
                                    >
                                        ...
                                        />
                                        <span className="truncate flex-1">{res.label ?? res.name}</span>
                                        {isActive && (
                    ...
                    {plugins?.some((p) => p.sidebarWidget) && (
                        <div className="mt-4 px-3 border-t border-[hsl(var(--border))] pt-4">
                            ...
                            {plugins.map(
                                (p, i) =>
                                    p.sidebarWidget && <p.sidebarWidget key={i} />
                            )}
                        </div>
                    )}
                </nav>
                ...
                <div className="border-t border-[hsl(var(--border))] p-3 space-y-1">
                    ...
                    <a
                        href="https://github.com/your-org/auto-admin"
                        target="_blank"
                        rel="noreferrer"
                        className={cn(
                            "flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium",
...
export function TopBar({ title }: TopBarProps) {
    ...
    return (
        <header
            className={cn(
                "fixed top-0 right-0 z-10 flex h-14 items-center gap-3 border-b border-[hsl(var(--border))]",
        ...
        >
            ...
            {title && (
                <span className="text-sm font-medium text-[hsl(var(--muted-foreground))] flex-1">{title}</span>
            )}
