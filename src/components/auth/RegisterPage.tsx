...
export function RegisterPage() {
  const { config, login, isLoading } = useAuth();
  const { t } = useI18n();
  ...
  }), [t.auth.name_min_length, t.auth.invalid_email, t.auth.password_min_length]);
  ...
  const onSubmit = async (data: RegisterFormValues) => {
    if (config?.provider?.register) {
      const { user, token } = await config.provider.register(data);
      login(user, token);
    } else {
      console.warn("Register endpoint not fully configured");
    }
  };
